...
pub const IMAGE_EXT:   &[&str] = &[
    ...
    // Raw camera formats (preview via ffmpeg thumb fallback)
    "raw","arw","cr2","cr3","nef","nrw","orf","raf","rw2","dng","pef","srw","x3f",
    // Other
    "heic","heif","jxl","svg","xcf","tga","dds","psd",
];
pub const VIDEO_EXT:   &[&str] = &["mp4","mkv","avi","mov","webm","flv","wmv","m4v","mpg","mpeg","ts","mts","m2ts","vob","ogv","3gp","3g2","rm","rmvb","divx","xvid","asf","f4v","hevc","h264","h265","264","265","mxf","dv","qt","amv"];
pub const AUDIO_EXT:   &[&str] = &["mp3","flac","ogg","wav","aac","m4a","opus","wma","ape","mka","aiff","aif","alac","dsd","dsf","dff","mid","midi","amr","tta","wv","caf","ra","au","snd","spx","mpc","ac3","dts","eac3","thd","truehd"];
...
/// Returns the local timezone offset in seconds by reading /etc/localtime

/// or the TZ environment variable. Falls back to 0 (UTC) on any error.

pub fn local_tz_offset_secs() -> i64 {

    // Use the C library's localtime to get the offset without pulling in chrono.

    // We compare what localtime says the current hour/minute is vs our UTC calculation.

    // Simple approach: parse /etc/localtime DST offset via reading POSIX TZ or

    // just call the libc gmtime/localtime pair via std::process if needed.

    // Most reliable no-dep method: read TZ offset from /etc/localtime symlink target

    // or from the `date +%z` command output.
    ...
    if let Ok(out) = Command::new("date").arg("+%z").output() {
        ...
        if s.len() >= 5 {
            ...
            if let (Ok(hh), Ok(mm)) = (digits[..2].parse::<i64>(), digits[2..4].parse::<i64>()) {
...
/// Returns current local time as seconds since UNIX epoch, timezone-adjusted.
...
pub fn local_secs() -> u64 {
    ...
    if offset >= 0 {
        ...
        utc.saturating_add(offset as u64)

    } else {

        utc.saturating_sub((-offset) as u64)
...
pub struct ExtractionProgress {
    ...
    pub pid:        Option<u32>,  // child process PID — set on first send, used for cancel
...
pub enum InputMode {
    ...
    OpenWithCustom(PathBuf),                          // typing a custom command
